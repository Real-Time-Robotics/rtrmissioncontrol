...
void TCPLink::stopReconnectTimer()
{
    if (_reconnectTimer->isActive()) {
        _reconnectTimer->stop();
    }
...
void TCPLink::_socketError(QAbstractSocket::SocketError socketError)
{
    ...
    if (!_socketIsConnected) {
        // Suppress warnings during initial connection and reconnect attempts
        return;
    }
    qWarning() << "Lost connection";
    QObject::disconnect(_socket, &QIODevice::readyRead, this, &TCPLink::_readBytes);
    _socket->disconnectFromHost();

    if (_socketIsConnected)
    {
        qWarning() << "Starting reconnection attempts...";
        _reconnectTimer->start(1000); // Start reconnect attempts
